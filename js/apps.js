                    ...
                    <div class="col-md-8 d-flex flex-column ">
                        ...
                        <div class="card-footer border-0 bg-body d-flex justify-content-between w-100">
                            ...
                            <div class="d-flex align-items-center">
                                <i onclick="loadModalDetail('${_id}')" class="fa-solid fa-arrow-right" data-bs-toggle="modal" data-bs-target="#exampleModal"></i>
                            </div>
                        </div>
                    </div>   
                </div>
...
const loadModalDetail = (news_id) => {
    ...
    fetch(url)
        .then(res => res.json())
        .then(data => showModalDetail(data.data[0]));
}
const showModalDetail = (data) => {
    console.log(data)
    const { title, details, thumbnail_url } = data;
    document.getElementById('exampleModalLabel').innerText = title;
    document.getElementById('modal-body').innerText = details;
    document.getElementById('modal-img').innerHTML = `<img src="${thumbnail_url}" class="img-fluid w-100 h-100 rounded-start" alt="...">`
}
